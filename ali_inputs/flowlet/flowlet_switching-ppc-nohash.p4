...
action lookup_flowlet_map() {
    modify_field(ingress_metadata.flowlet_map_index, 1);
...
action set_ecmp_select(ecmp_base, ecmp_count) {
    modify_field(ingress_metadata.ecmp_offset, ecmp_base);
    add_to_field(ingress_metadata.ecmp_offset, ecmp_count);
}
table ecmp_group {
    reads {
        ipv4.dstAddr : lpm;
